...
function update_database($page_info){
    ...
    foreach ($page_info as $item) {
        ...
        (`sysid`,`supplier_id`,`page_id`,`row`,`lib_id`,`lib_title`,`img_url`,`item_url`,`lib_num`,`lib_num_max`)
        values('".$item['sysid']."',".$item['supplier_id'].",".$item['page_id'].",".$item['row'].",".$item['lib_id'].",'".$item['lib_title']."',".
            "'".$item['img_url']."','". $item['item_url']."',". $item['lib_num'].",".$item['lib_num_max'].")";

      //  weixindebug($sql,'$sql',0,'','','','admin/update_info.php:13',13);
